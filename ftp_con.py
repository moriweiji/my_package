...
class MYFTP:
    ...
    def download_file(self, localfile, remotefile):
        if self.is_same_size(localfile, remotefile):
            ...
            self.change_file_list.append(localfile)
...
def ftp_down_main():
    ...
    password = ''  # 密码
    port = 21  # 端口号
    rootdir_local = './result'  # 本地目录
    rootdir_remote = '/'  # 远程目录

    f = MYFTP(hostaddr, username, password, rootdir_remote, port)
    f.login()
    result = f.download_files(rootdir_local, rootdir_remote)
    timenow = time.localtime()
    datenow = time.strftime('%Y-%m-%d', timenow)
    logstr = u"%s 成功执行了备份\n" % datenow
    debug_print(logstr)
...
def ftp_update_main(filename):
    ...
    f = MYFTP(hostaddr, username, password, rootdir_remote, port)
    f.login()
    localfile = '../result/%s' % filename
    remotefile = filename
    result = f.upload_file(localfile,remotefile)
    timenow = time.localtime()
    datenow = time.strftime('%Y-%m-%d', timenow)
    logstr = u"%s 成功执行了上传\n" % datenow
    debug_print(logstr)
